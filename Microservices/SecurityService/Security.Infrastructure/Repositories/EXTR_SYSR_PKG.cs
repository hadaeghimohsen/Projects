...
{
    public class EXTR_SYSR_PKG : IEXTR_SYSR_SRVC
    {
        private readonly string _connectionString;

        public EXTR_SYSR_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_SYSR_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_SYSR_U(string p_in)
            => await ExecuteFunctionAsync("INIT_SYSR_U", p_in);

        public async Task<string> LIST_SYSR_U(string p_in)
            => await ExecuteFunctionAsync("LIST_SYSR_U", p_in);
    }
