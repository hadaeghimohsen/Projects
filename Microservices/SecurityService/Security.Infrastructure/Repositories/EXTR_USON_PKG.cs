...
{
    public class EXTR_USON_PKG : IEXTR_USON_SRVC
    {
        private readonly string _connectionString;

        public EXTR_USON_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_USON_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_USON_U(string p_in)
            => await ExecuteFunctionAsync("INIT_USON_U", p_in);

        public async Task<string> CRET_USON_U(string p_in)
            => await ExecuteFunctionAsync("CRET_USON_U", p_in);

        public async Task<string> LOUT_USON_U(string p_in)
            => await ExecuteFunctionAsync("LOUT_USON_U", p_in);

        public async Task<string> LSEN_USON_U(string p_in)
            => await ExecuteFunctionAsync("LSEN_USON_U", p_in);

        public async Task<string> ACTV_USON_U(string p_in)
            => await ExecuteFunctionAsync("ACTV_USON_U", p_in);

        public async Task<string> DACT_USON_U(string p_in)
            => await ExecuteFunctionAsync("DACT_USON_U", p_in);

        public async Task<string> LIST_USON_U(string p_in)
            => await ExecuteFunctionAsync("LIST_USON_U", p_in);
    }
}
