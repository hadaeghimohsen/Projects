...
{
    public class EXTR_URTK_PKG : IEXTR_URTK_SRVC
    {
        private readonly string _connectionString;

        public EXTR_URTK_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_URTK_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_URTK_U(string p_in)
            => await ExecuteFunctionAsync("INIT_URTK_U", p_in);

        public async Task<string> LIST_URTK_U(string p_in)
            => await ExecuteFunctionAsync("LIST_URTK_U", p_in);
    }
}
