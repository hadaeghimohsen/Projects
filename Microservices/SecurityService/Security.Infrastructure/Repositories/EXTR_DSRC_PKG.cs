...
{
    public class EXTR_DSRC_PKG : IEXTR_DSRC_SRVC
    {
        private readonly string _connectionString;

        public EXTR_DSRC_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_DSRC_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_DSRC_U(string p_in)
            => await ExecuteFunctionAsync("INIT_DSRC_U", p_in);

        public async Task<string> CRET_DSRC_U(string p_in)
            => await ExecuteFunctionAsync("CRET_DSRC_U", p_in);

        public async Task<string> REMV_DSRC_U(string p_in)
            => await ExecuteFunctionAsync("REMV_DSRC_U", p_in);

        public async Task<string> UPDT_DSRC_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_DSRC_U", p_in);

        public async Task<string> LIST_DSRC_U(string p_in)
            => await ExecuteFunctionAsync("LIST_DSRC_U", p_in);
    }
}
