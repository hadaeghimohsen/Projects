...
{
    public class EXTR_BOXP_PKG : IEXTR_BOXP_SRVC
    {
        private readonly string _connectionString;

        public EXTR_BOXP_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_BOXP_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_BOXP_U(string p_in)
            => await ExecuteFunctionAsync("INIT_BOXP_U", p_in);

        public async Task<string> CRET_BOXP_U(string p_in)
            => await ExecuteFunctionAsync("CRET_BOXP_U", p_in);

        public async Task<string> REMV_BOXP_U(string p_in)
            => await ExecuteFunctionAsync("REMV_BOXP_U", p_in);

        public async Task<string> UPDT_BOXP_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_BOXP_U", p_in);

        public async Task<string> LIST_BOXP_U(string p_in)
            => await ExecuteFunctionAsync("LIST_BOXP_U", p_in);
    }
}
