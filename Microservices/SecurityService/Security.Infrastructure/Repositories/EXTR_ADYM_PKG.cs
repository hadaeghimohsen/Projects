...
{
    public class EXTR_ADYM_PKG : IEXTR_ADYM_SRVC
    {
        private readonly string _connectionString;

        public EXTR_ADYM_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_ADYM_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_ADYM_U(string p_in)
            => await ExecuteFunctionAsync("INIT_ADYM_U", p_in);

        public async Task<string> CRET_ADYM_U(string p_in)
            => await ExecuteFunctionAsync("CRET_ADYM_U", p_in);

        public async Task<string> REMV_ADYM_U(string p_in)
            => await ExecuteFunctionAsync("REMV_ADYM_U", p_in);

        public async Task<string> UPDT_ADYM_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_ADYM_U", p_in);

        public async Task<string> LIST_ADYM_U(string p_in)
            => await ExecuteFunctionAsync("LIST_ADYM_U", p_in);
    }
}
