...
{
    public class EXTR_ROLE_PKG : IEXTR_ROLE_SRVC
    {
        private readonly string _connectionString;

        public EXTR_ROLE_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_ROLE_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("INIT_ROLE_U", p_in);

        public async Task<string> CRET_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("CRET_ROLE_U", p_in);

        public async Task<string> REMV_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("REMV_ROLE_U", p_in);

        public async Task<string> UPDT_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_ROLE_U", p_in);

        public async Task<string> ACTV_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("ACTV_ROLE_U", p_in);

        public async Task<string> DACT_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("DACT_ROLE_U", p_in);

        public async Task<string> DUPL_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("DUPL_ROLE_U", p_in);

        public async Task<string> LIST_ROLE_U(string p_in)
            => await ExecuteFunctionAsync("LIST_ROLE_U", p_in);
    }
}
