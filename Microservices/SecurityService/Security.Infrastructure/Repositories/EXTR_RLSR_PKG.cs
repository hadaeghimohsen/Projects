...
{
    public class EXTR_RLSR_PKG : IEXTR_RLSR_SRVC
    {
        private readonly string _connectionString;

        public EXTR_RLSR_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_RLSR_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_RLSR_U(string p_in)
            => await ExecuteFunctionAsync("INIT_RLSR_U", p_in);

        public async Task<string> LIST_RLSR_U(string p_in)
            => await ExecuteFunctionAsync("LIST_RLSR_U", p_in);
    }
}
