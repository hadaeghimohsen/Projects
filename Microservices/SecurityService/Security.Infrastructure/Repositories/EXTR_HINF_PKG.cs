...
{
    public class EXTR_HINF_PKG : IEXTR_HINF_SRVC
    {
        private readonly string _connectionString;

        public EXTR_HINF_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_HINF_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_HINF_U(string p_in)
            => await ExecuteFunctionAsync("INIT_HINF_U", p_in);

        public async Task<string> REGS_HINF_U(string p_in)
            => await ExecuteFunctionAsync("REGS_HINF_U", p_in);

        public async Task<string> LIST_HINF_U(string p_in)
            => await ExecuteFunctionAsync("LIST_HINF_U", p_in);
    }
}
