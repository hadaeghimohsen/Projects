...
{
    public class EXTR_SYS_PKG : IEXTR_SYS_SRVC
    {
        private readonly string _connectionString;

        public EXTR_SYS_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_SYS_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_SYS_U(string p_in)
            => await ExecuteFunctionAsync("INIT_SYS_U", p_in);

        public async Task<string> CRET_SYS_U(string p_in)
            => await ExecuteFunctionAsync("CRET_SYS_U", p_in);

        public async Task<string> REMV_SYS_U(string p_in)
            => await ExecuteFunctionAsync("REMV_SYS_U", p_in);

        public async Task<string> UPDT_SYS_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_SYS_U", p_in);

        public async Task<string> LIST_SYS_U(string p_in)
            => await ExecuteFunctionAsync("LIST_SYS_U", p_in);
    }
}
