...
{
    public class EXTR_PRVL_PKG : IEXTR_PRVL_SRVC
    {
        private readonly string _connectionString;

        public EXTR_PRVL_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_PRVL_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("INIT_PRVL_U", p_in);

        public async Task<string> CRET_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("CRET_PRVL_U", p_in);

        public async Task<string> REMV_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("REMV_PRVL_U", p_in);

        public async Task<string> UPDT_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_PRVL_U", p_in);

        public async Task<string> ACTV_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("ACTV_PRVL_U", p_in);

        public async Task<string> DACT_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("DACT_PRVL_U", p_in);

        public async Task<string> LIST_PRVL_U(string p_in)
            => await ExecuteFunctionAsync("LIST_PRVL_U", p_in);
    }
}
