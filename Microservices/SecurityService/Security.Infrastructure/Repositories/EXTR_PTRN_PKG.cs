...
{
    public class EXTR_PTRN_PKG : IEXTR_PTRN_SRVC
    {
        private readonly string _connectionString;

        public EXTR_PTRN_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_PTRN_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_PTRN_U(string p_in)
            => await ExecuteFunctionAsync("INIT_PTRN_U", p_in);

        public async Task<string> LIST_PTRN_U(string p_in)
            => await ExecuteFunctionAsync("LIST_PTRN_U", p_in);
    }
}
