...
{
    public class EXTR_PGAT_PKG : IEXTR_PGAT_SRVC
    {
        private readonly string _connectionString;

        public EXTR_PGAT_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_PGAT_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_PGAT_U(string p_in)
            => await ExecuteFunctionAsync("INIT_PGAT_U", p_in);

        public async Task<string> LIST_PGAT_U(string p_in)
            => await ExecuteFunctionAsync("LIST_PGAT_U", p_in);
    }
}
