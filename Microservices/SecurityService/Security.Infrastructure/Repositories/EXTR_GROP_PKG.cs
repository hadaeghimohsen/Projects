...
{
    public class EXTR_GROP_PKG : IEXTR_GROP_SRVC
    {
        private readonly string _connectionString;

        public EXTR_GROP_PKG(string connectionString)
        {
        ...
        private async Task<string> ExecuteFunctionAsync(string functionName, string jsonInput)
        {
            ...
            using var command = new OracleCommand($"EXTR_GROP_PKG.{functionName}", connection)
            {
        ...
        public async Task<string> INIT_GROP_U(string p_in)
            => await ExecuteFunctionAsync("INIT_GROP_U", p_in);

        public async Task<string> CRET_GROP_U(string p_in)
            => await ExecuteFunctionAsync("CRET_GROP_U", p_in);

        public async Task<string> REMV_GROP_U(string p_in)
            => await ExecuteFunctionAsync("REMV_GROP_U", p_in);

        public async Task<string> UPDT_GROP_U(string p_in)
            => await ExecuteFunctionAsync("UPDT_GROP_U", p_in);

        public async Task<string> LIST_GROP_U(string p_in)
            => await ExecuteFunctionAsync("LIST_GROP_U", p_in);
    }
}
